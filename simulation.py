...
class TransferModel(nn.Module):
    ...
    def __init__(self, model_name, transfer, num_input_channel, num_output_class, dropout):
        ...
        if model_name in ['Xception']:
            input_layer = nn.Conv2d(num_input_channel, self.model.conv1.out_channels, kernel_size=3,
            ...
            input_layer = nn.Conv2d(num_input_channel, self.model._conv_stem.out_channels, kernel_size=3,
                                    stride=1, padding=1, dilation=1, groups=1, bias=True)
...
def model_run(model_name, transfer, noise):
    ...
    for i in range(5):
        ...
        for e in range(0, epochs):
            ...
            if not best_valid_loss or valid_loss < best_valid_loss:
                ...
                torch.save(model.state_dict(),
                           os.path.join(os.getcwd(), 'snapshot', '{}_{}'.format(model_name, transfer), out_name))
        ...
        test_result_path = os.path.join(os.getcwd(), 'results', '{}_{}'.format(model_name, transfer),
                                        str('test_result_{:0.2f}_{}.pkl'.format(noise, i)))
